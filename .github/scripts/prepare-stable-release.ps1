...
$version = & (Join-Path $PSScriptRoot 'get-release-version.ps1')

$tag = "v$version"
if ($env:GITHUB_REF -like 'refs/tags/*' -and $env:GITHUB_REF -ne "refs/tags/$tag") {
    throw 'The requested tag must match Program.AppVersion exactly.'
}
