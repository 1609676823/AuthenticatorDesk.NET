...
release_script="$repository_root/.github/scripts/publish-release.sh"
validation_root="$repository_root/artifacts/local-validation"
...
gh() {
  ...
  case "$*" in
    ...
    'release view '*)
      ...
      printf 'AuthenticatorDesk-%s-win-x64-self-contained.zip\n' "$RELEASE_TAG"
      printf 'AuthenticatorDesk-%s-win-x64.zip\n' "$RELEASE_TAG"
      printf 'unrelated-notes.pdf\n' ;;
    'release delete-asset '*)
      if [[ "$MOCK_CASE" == 'force_stable_delete_failure' ]]; then return 26; fi
      [[ "$*" == "release delete-asset $RELEASE_TAG AuthenticatorDesk-$RELEASE_TAG-win-x64.zip --yes" ]] || return 98 ;;
...
for scenario in nightly_new nightly_existing nightly_immutable nightly_wrong_channel upload_failure ref_failure api_failure stable_new stable_without_tag stable_published stable_draft stable_wrong_channel stable_moved_tag invalid_tag checksum_failure force_stable_existing force_stable_draft force_stable_new force_stable_without_tag force_stable_immutable force_stable_wrong_channel force_stable_upload_failure force_stable_ref_failure force_stable_assets_failure force_stable_delete_failure force_stable_publish_failure force_stable_schedule force_stable_push invalid_force; do
  ...
  : > "$MOCK_LOG"
  printf 'test package\n' > "$case_dir/artifacts/AuthenticatorDesk-$RELEASE_TAG-win-x64-self-contained.zip"
  printf 'portable package\n' > "$case_dir/artifacts/AuthenticatorDesk-$RELEASE_TAG-win-portable-framework-dependent.zip"
  printf 'test notes\n' > "$case_dir/artifacts/release-notes.md"
  (cd "$case_dir/artifacts"; sha256sum --text "AuthenticatorDesk-$RELEASE_TAG-win-x64-self-contained.zip" "AuthenticatorDesk-$RELEASE_TAG-win-portable-framework-dependent.zip" > SHA256SUMS.txt)
  ...
  case "$scenario" in
    ...
    force_stable_existing|force_stable_draft|force_stable_new|force_stable_without_tag)
      grep -q '^release upload .*--clobber$' "$MOCK_LOG"
      grep -q '^release delete-asset v0.1.0 AuthenticatorDesk-v0.1.0-win-x64.zip --yes$' "$MOCK_LOG"
