...
if not exist "%REPO_ROOT%\AuthenticatorDesk.NET.slnx" goto :wrong_project
if not exist "%REPO_ROOT%\AuthenticatorDesk.NET\AuthenticatorDesk.NET.csproj" goto :wrong_project
...
rem Validate path ancestry and Git metadata before inspecting or removing them.
call :check_git_metadata
if errorlevel 1 goto :unsafe_git_dir
...
call :check_git_metadata remove
if errorlevel 1 goto :unsafe_git_dir
...
:wrong_project
echo ERROR: Required AuthenticatorDesk.NET project files were not found beside this script.
call :pause_before_exit
endlocal & exit /b 1
...
:check_git_metadata
rem Recheck the absolute target immediately before removal; never traverse links.
set "REMOVE_GIT_METADATA=%~1"
powershell.exe -NoProfile -Command ^
    "$ErrorActionPreference = 'Stop'; try {" ^
    "$rootPath = [IO.Path]::GetFullPath($env:REPO_ROOT); $root = Get-Item -LiteralPath $rootPath -Force;" ^
    "if (-not $root.PSIsContainer -or $root.FullName -eq $root.Root.FullName) { throw 'Invalid repository root.' };" ^
    "for ($ancestor = $root; $null -ne $ancestor; $ancestor = $ancestor.Parent) { if (($ancestor.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw ('Repository path contains a reparse point: ' + $ancestor.FullName) } };" ^
    "$expected = [IO.Path]::GetFullPath((Join-Path $rootPath '.git')); $target = [IO.Path]::GetFullPath($env:TARGET_GIT_DIR);" ^
    "if (-not [string]::Equals($target, $expected, [StringComparison]::OrdinalIgnoreCase)) { throw 'Git metadata is outside the expected repository path.' };" ^
    "$git = Get-Item -LiteralPath $target -Force -ErrorAction SilentlyContinue; if ($null -eq $git) { if (Test-Path -LiteralPath $target) { throw 'Git metadata cannot be inspected.' }; exit 0 };" ^
    "if (-not $git.PSIsContainer -or ($git.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw 'Git metadata must be an ordinary directory.' };" ^
    "$pending = New-Object System.Collections.Stack; $pending.Push($git);" ^
    "while ($pending.Count -gt 0) { $directory = $pending.Pop(); foreach ($item in @(Get-ChildItem -LiteralPath $directory.FullName -Force)) { if (($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw ('Git metadata contains a reparse point: ' + $item.FullName) }; if ($item.PSIsContainer) { $pending.Push($item) } } };" ^
    "if ($env:REMOVE_GIT_METADATA -eq 'remove') { Remove-Item -LiteralPath $target -Recurse -Force }; exit 0" ^
